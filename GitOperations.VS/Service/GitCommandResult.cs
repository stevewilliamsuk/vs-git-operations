    ...
    public class GitTimedOutCommandResult : GitCommandResult
    {
        public GitTimedOutCommandResult(string command)
            : base(
                false,
                "The command '" + command +
                "' is taking longer than expected. You might be prompted for information such as credentials. Please run the command from command line to find out what is blocking the process"
                )
        {
        }
    }
